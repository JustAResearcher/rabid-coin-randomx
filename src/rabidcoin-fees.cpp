...
CFeeRate GetRabidcoinFeeRate(int priority)
{
    switch(priority)
    {
    case SUCH_EXPENSIVE:
...
const std::string GetRabidcoinPriorityLabel(int priority)
{
    switch(priority)
    {
    case SUCH_EXPENSIVE:
...
CAmount GetRabidcoinMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    {
        LOCK(mempool.cs);
    ...
    nMinFee += GetRabidcoinDustFee(tx.vout, nDustLimit);
...
CAmount GetRabidcoinDustFee(const std::vector<CTxOut> &vout, const CAmount dustLimit) {
    CAmount nFee = 0;
