...
/*
 * RabidCoin: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * RabidCoin: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * RabidCoin: Get a fee targeting a specific transaction speed.
     */
    CAmount GetRabidCoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * RabidCoin: Get a fee targeting a specific transaction speed.
     */
    static CAmount GetRabidCoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the RabidCoin dust fee.
     */
