...
#if defined(NDEBUG)
# error "Rabidcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Rabidcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetRabidcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Rabidcoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("rabidcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Rabidcoin: BIP30 has been active since inception
    ...
    // Rabidcoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetRabidcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Rabidcoin: Disallow AuxPow blocks before it is activated.
    ...
    // Rabidcoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Rabidcoin: We probably want to disable this
    int nLockTimeFlags = 0;
